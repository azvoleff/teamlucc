...
glcm <- function(layer, n_grey=32, window=c(3, 3), shift=c(1, 1),
    ...
    base_indices <- as.vector((base_cols - 1) * (window[1]+abs(shift[1])) + 
                              base_rows)
    center_coord <- ul_base + abs(shift)

    if (useRasterEngine) {
        # Add a wrapper for calc_texture function so that rasterEngine can pass 
        # ...  arguments to it (this avoids having to modify the auto-generated 
        # wrapper for the C++ calc_texture function that is generated by Rcpp).
        ...
        texture_img <- rasterEngine(rast=layer, fun=calc_texture_wrapper,
