...
Rcpp::NumericVector calc_texture(arma::mat Rast,
        Rcpp::CharacterVector statistics, arma::uvec base_indices,
        arma::uvec offset_indices, int n_grey) {
    ...
    for(unsigned i=0; i < offset_indices.size(); i++) {
        ...
        pij(Rast(base_indices(i)) - 1, Rast(offset_indices(i)) - 1)++;
    }
    pij = pij / base_indices.n_elem;
    ...
    mu = sum(Rast(base_indices)) / base_indices.n_elem;
