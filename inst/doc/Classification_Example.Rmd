...
slopeaspect <- slopeasp_seq(matched_DEM, smoothing=1.5)
```
...
## Setup predictors for SVM classification
...
```{r tm_1986_classification}
train_data_1986 <- extract_training_data(L5TSR_1986_preds, 
                                         L5TSR_1986_2001_training, "t1_class", 
                                         testfrac=.3)
svm_1986 <- svm_classify(L5TSR_1986_preds, train_data_1986)
svm_1986$svm_train
plot(svm_1986$pred_classes)
```

Perform accuracy assessment using an independent dataset from the training data:

```{r tm_1986_class_accuracy}
accuracy(svm_1986$svm_train, train_data_1986)
```
