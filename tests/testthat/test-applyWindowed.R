...
###############################################################################
# Test that glcm calculated with apply_windowed matches the output from running 
# glcm directly
...
test_that("glcm calculated with apply_windowed matches glcm calculated directly", {
    expect_equal(glcm_apply_windowed, expected=glcm_glcm)
})
...
glcm_apply_windowed_single <- apply_windowed(x, glcm, edge=c(1, 2), min_x=min_x, 
