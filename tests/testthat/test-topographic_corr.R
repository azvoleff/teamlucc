...
###############################################################################
# Test that minslope methods match between landsat package 'topocorr' and teamlucc 
# 'topographic_corr':
teamlucc_tc_b1 <- topographic_corr(L5TSR_1986_b1, slopeaspect, sunelev, sunazimuth, 
                                method='minslope')
...
test_that("teamlucc and landsat minslope match", {
          expect_equal(teamlucc_tc_b1, expected=landsat_tc_b1)
})

###############################################################################
set.seed(1)
sampleindices <- gridsample(L5TSR_1986_b1, rowmajor=TRUE)
teamlucc_tc_b1_sample <- topographic_corr(L5TSR_1986_b1, slopeaspect, sunelev, 
                                       sunazimuth, method='minslope', 
                                       sampleindices=sampleindices)

test_that("teamlucc and landsat minslope match when sampling is used in teamlucc", {
          expect_equal(teamlucc_tc_b1_sample, expected=landsat_tc_b1, tolerance=.25)
})
...
teamlucc_tc_b1_b2 <- topographic_corr(stack(L5TSR_1986_b1, L5TSR_1986_b2),
...
test_that("teamlucc and landsat minslope match when multiple layers are processed in teamlucc", {
          expect_equal(teamlucc_tc_b1_b2, expected=landsat_tc_b1_b2)
})

###############################################################################
# Test that minnaert_full methods match between landsat package 'topocorr' and 
# teamlucc 'topographic_corr' when using full image.
teamlucc_minnaert <- topographic_corr(L5TSR_1986_b1, slopeaspect, sunelev, 
...
test_that("teamlucc minnaert and landsat minnaert match", {
          expect_equal(teamlucc_minnaert, expected=landsat_minnaert, tolerance=.1)
})

###############################################################################
# Test that minnaert_full methods match between landsat package 'topocorr' and 
# teamlucc 'topographic_corr' when using resampling.
set.seed(0)
sampleindices <- gridsample(L5TSR_1986_b1, rowmajor=TRUE)
teamlucc_minnaert_sample <- topographic_corr(L5TSR_1986_b1, slopeaspect, sunelev, 
                                          sunazimuth, method='minnaert_full',
                                          sampleindices=sampleindices,
                                          slopeclass=c(1, 5, 10, 15, 20, 25, 30, 45))

test_that("teamlucc minnaert sample and landsat minnaert match", {
          expect_equal(teamlucc_minnaert_sample, expected=landsat_minnaert, 
                       tolerance=.25)
})

###############################################################################
# Test that minnaert_full methods match when teamlucc minnaert_full runs 
...
# sfQuickInit(2)
#
# teamlucc_minnaert_sample_b1b2 <- topographic_corr(stack(L5TSR_1986_b1, L5TSR_1986_b2),
#                                               slopeaspect, sunelev, sunazimuth, 
#                                               method='minnaert_full',
#                                               sampleindices=sampleindices)
#
# teamlucc_minnaert_sample_b1b2_par <- topographic_corr(stack(L5TSR_1986_b1, L5TSR_1986_b2),
#                                               slopeaspect, sunelev, sunazimuth, 
#                                               method='minnaert_full',
#                                               sampleindices=sampleindices,
#                                               inparallel=TRUE)
...
#
# test_that("teamlucc minnaert sample and landsat minnaert match", {
#           expect_equal(teamlucc_minnaert_sample_b1b2, expected=teamlucc_minnaert_sample_b1b2_par)
# })
